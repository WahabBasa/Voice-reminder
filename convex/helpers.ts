...
// Keep-what-matters, not shortest-wins: lines that escaped the opener ban used
// to pad ('Hydrate yourself with a fresh glass of water now'), but a hard word
// budget over-corrected and trimmed real details the user gave — 'take the
// water bottle out of the fridge' came back as 'Take out your water bottle',
// dropping the place. The rule now keeps every concrete detail that changes what
// the user would physically do and cuts only filler, so length follows the
// input instead of a fixed cap. The canon examples are the product decision made
// flesh (OLD-104): the same task as a bare imperative, the same event as '[X] is
// right now', no name and no framing in any of them; the qualifier examples show
// a kept detail. Single-quoted throughout — this string is embedded inside a
// double-quoted JSON field in the prompt.
export function buildDescriptionInstruction(): string {
  return `the sentence spoken aloud when the reminder fires, in the input's language. ${SPOKEN_LINE_RULE_REFERENCE} Keep every concrete detail the user gave that changes what they would physically do — the place, the source, the destination, the quantity, which one, or who it involves — and drop only framing words, filler, and commentary. As short as the user's own details allow: no fixed word count, and never cut a real detail to hit one, but no filler adjectives and no dressing up a simple task either. Never add wellness, benefit or encouragement commentary. The wording must still be true if it is heard a few minutes late, so avoid countdowns like 'in 10 minutes'. When the task is bare, the line stays bare: 'Drink your water.' / 'Take your pills.' / 'Call your mother.' / 'Your son's game is right now.' / 'Your meeting with Ahmed is right now.' When the user named a detail, keep it in the line: 'Take the water bottle out of the fridge.' / 'Move the laundry to the dryer.' / 'Call Ahmed about the invoice.' The same in Arabic: 'اشرب ماءك.' / 'خذ حبوبك.' / 'اتصل بأمك.' / 'مباراة ابنك الآن.' / 'اجتماعك مع أحمد الآن.' / 'أخرج قارورة الماء من الثلاجة.' / 'انقل الغسيل إلى النشافة.'`;
}
